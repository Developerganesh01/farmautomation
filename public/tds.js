...
const CollectionModel=require('../models/tdsModel');
const path=require('path');
const express=require('express');
const tds_sensor_router=express.Router();
// ph_sensor_router.get('/',(req,res)=>{
//     res.status(200).send('phsensor');
// })
tds_sensor_router.get('/',(req,res)=>{
    res.status(200).sendFile(path.join(__dirname,'..','/public','ph.html'));
});
//data retrive
// Define a route to fetch data
tds_sensor_router.get('/data', async (req, res) => {
    try {
...
module.exports=tds_sensor_router;
